...
import { LayoutDashboard, Briefcase, Users, Gavel, FileText, BrainCircuit, LogOut, Menu, Bell, Calendar, X, Clock, AlertTriangle, CheckCircle, ChevronRight, ChevronLeft, Settings, BarChart3, Wallet, File, Search, Library, PlusCircle, Shield, Crown } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, activePage, onNavigate, notifications = [], onNotificationClick, currentUser, onLogout }) => {
  ...
  const getBadgeCount = (id: string) => {
    // Filter notifications to only include High or Critical urgency for badges
    // This prevents showing a badge for a hearing 7 days away, keeping it "Real" for today/overdue items.
      ...
      `}>
        ...
        <nav className="flex-1 px-3 py-2 space-y-6 overflow-y-auto overflow-x-hidden custom-scrollbar">
          {navGroups.map((group, groupIndex) => {
             // Filter items in group based on permissions
             const visibleItems = group.items.filter(item => checkPermission(item.id));
        ...
        <div className="border-t border-slate-800 bg-slate-900/50 p-2">
           <div className={`flex items-center gap-3 p-2 rounded-xl hover:bg-slate-800 transition-colors cursor-pointer group ${isSidebarCollapsed ? 'justify-center' : ''}`}>
              <div className="relative">
                 <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg ring-2 ring-slate-800 overflow-hidden ${AuthService.isAdmin(currentUser) ? 'bg-gradient-to-tr from-yellow-500 to-orange-600' : 'bg-gradient-to-tr from-indigo-500 to-purple-600'}`}>
                    {AuthService.isAdmin(currentUser) ? <Crown className="w-5 h-5" /> : (currentUser?.avatar ? <img src={currentUser.avatar} className="w-full h-full object-cover" /> : currentUser?.name.charAt(0))}
                 </div>
                 <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-slate-900 rounded-full"></span>
                 {AuthService.isAdmin(currentUser) && (
                    <span className="absolute -top-1 -right-1 w-3 h-3 bg-yellow-500 border-2 border-slate-900 rounded-full flex items-center justify-center">
              ...
              {!isSidebarCollapsed && (
                 <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-white truncate flex items-center gap-2">
                       {currentUser?.name}
                       {AuthService.isAdmin(currentUser) && <Crown className="w-3 h-3 text-yellow-400" />}
                    </p>
                    <p className="text-[10px] text-slate-400 truncate">{AuthService.getRoleDisplayName(currentUser)}</p>
                 </div>
              )}
