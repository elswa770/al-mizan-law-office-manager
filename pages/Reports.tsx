...
import { 
  ...
  Users, AlertCircle, Calendar, Printer, Filter, PieChart as PieIcon, Download, Building2, FileText
} from 'lucide-react';
...
const Reports: React.FC<ReportsProps> = ({ cases, clients, hearings, tasks }) => {
  ...
  const caseStats = useMemo(() => {
    ...
    const courtData = Object.keys(courtCounts).map(key => ({ name: key, value: courtCounts[key] }));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 print:block print:space-y-6">
           ...
           <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 break-inside-avoid print:break-inside-avoid print:border-slate-300">
              ...
              <div className="h-64 w-full text-xs print:h-[250px]">
                 <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={operationalStats.hearingData} layout="vertical" margin={{ left: 20 }}>
                       ...
                       <Tooltip cursor={{fill: 'transparent'}} wrapperClassName="print:hidden" contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', borderRadius: '8px', border: 'none' }} />
                       <Bar dataKey="value" fill="#6366f1" radius={[0, 4, 4, 0]} barSize={20} name="عدد الجلسات" isAnimationActive={false} />
                    </BarChart>
                 </ResponsiveContainer>
              </div>
           </div>
           ...
           <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 break-inside-avoid print:break-inside-avoid print:border-slate-300 print:mt-6">
              <h3 className="font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                 <Building2 className="w-5 h-5 text-slate-600 dark:text-slate-400 print:text-black" />
