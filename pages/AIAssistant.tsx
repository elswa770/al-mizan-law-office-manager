...
interface AIAssistantProps {
  cases?: Case[];
  references?: LegalReference[]; // New Prop
  onUpdateCase?: (updatedCase: Case) => void;
}

const AIAssistant: React.FC<AIAssistantProps> = ({ cases = [], references = [], onUpdateCase }) => {
  // --- Chat State ---
  ...
  const getContextString = () => {
    ...
    if (selectedContextCaseId) {
      ...
      if (c) {
        ...
        // Deep Strategy
        if (c.strategy) {
           context += `\n\n=== استراتيجية الدفاع الخاصة بنا (سري للغاية) ===\n`;
        ...
        // Memos (Titles & Types) - In a real app, we'd send content too
    ...
    // Injecting relevant library items allows the AI to "learn" from what the lawyer has saved.
    if (references.length > 0) {
       context += `\n\n=== مكتبة المراجع القانونية الخاصة بالمكتب (للاسترشاد) ===\n`;
       // Limit to avoid token overflow, prioritize by relevance if possible, currently taking first 20 titles/descriptions
          ...
          {selectedContextCaseId && (
             <div className="mt-2 p-2 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 rounded text-xs text-indigo-700 dark:text-indigo-300 animate-in fade-in">
                ...
                <p className="mt-1 opacity-80 leading-relaxed">
                   يتم الآن استخدام الاستراتيجية، الملاحظات، بيانات الخصوم، والمكتبة القانونية في الردود.
                </p>
             </div>
          )}
          {references.length > 0 && (
