...
import { Calendar, MapPin, Gavel, AlertCircle, X, Edit3, Link as LinkIcon, ExternalLink, ChevronLeft, ChevronRight, List, LayoutGrid, Clock, Filter, Printer, Download, Plus, CheckSquare, AlignJustify, DollarSign, CalendarDays, ArrowLeftCircle, CheckCircle, FileText, Upload, Image as ImageIcon, Eye } from 'lucide-react';
...
const Hearings: React.FC<HearingsProps> = ({ hearings, cases, onCaseClick, onUpdateHearing, onAddHearing }) => {
  // --- View State ---
       ...
       days.push(
          <div key={d} className={`min-h-[100px] bg-white dark:bg-slate-800 border-r border-b border-slate-200 dark:border-slate-700 p-2 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors ${isToday ? 'bg-blue-50/30 dark:bg-blue-900/10' : ''}`}>
             ...
             <div className="space-y-1">
                {dayHearings.map(h => (
                   <div key={h.id} className="text-[10px] p-1 rounded bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-800 truncate cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors" onClick={() => openWizard(h)}>
                  ...
                  {wizardStep === 1 && (
                     <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                        ...
                        <div>
                           ...
                           />
                           <div 
                              onClick={() => rulingFileRef.current?.click()}
                              className="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-6 text-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 hover:border-primary-400 transition-colors flex flex-col items-center gap-2"
                           >
                     ...
                     <div className="space-y-5 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-800 flex items-center justify-between">
                           ...
                           <label className="relative inline-flex items-center cursor-pointer">
                              <input type="checkbox" className="sr-only peer" checked={nextSessionData.createNext} onChange={e => setNextSessionData({...nextSessionData, createNext: e.target.checked})} />
